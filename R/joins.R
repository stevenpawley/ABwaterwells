...
awwid_entity_name <- function(x, nav) {
  ...
  if (!inherits(x, "awwid")) {
    abort(
      "`x` must be an awwid tibble (from awwid_tbl()) or a character entity name."
    )
  }
...
awwid_join <- function(x, y, nav, type = "left") {
  ...
  xy <- nav[[x_entity]]
  xy <- xy[
    !is.na(xy$local_property) & xy$related_entity == y_entity,
    ,
    drop = FALSE
  ]
  ...
  yx <- nav[[y_entity]]
  yx <- yx[
    !is.na(yx$local_property) & yx$related_entity == x_entity,
    ,
    drop = FALSE
  ]
  ...
  if (nrow(xy) > 0) {
    ...
    by <- stats::setNames(
      tolower(xy$referenced_property),
      tolower(xy$local_property)
    )
  } else if (nrow(yx) > 0) {
    # y.local_property = x.referenced_property  (flip for dplyr: names=x, values=y)
    by <- stats::setNames(
      tolower(yx$local_property),
      tolower(yx$referenced_property)
    )
  } else {
    abort(glue(
