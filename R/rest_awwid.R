...
#' @export
rest_awwid = R6::R6Class(
  classname = "rest_awwid",
  public = list(
    ...
    request = function(name, filter = NULL, select = NULL, top = NULL) {
      ...
      tbl = awwid_tbl$new(name = name, x = df, request = request_url)
      return(tbl)
    },
    ...
    query_lithologs = function(ext = NULL) {
      ...
      well_reports = self$request("wellreports", select = report_cols, filter = filter_query)$standardize()
      ...
      lithologies = self$request("lithologies", select = lithologies_cols, filter = filter_query)$standardize()
  ...
  private = list(
    ...
    request_wells_geographic = function(ext) {
      ...
      self$request("wells", select = wells_cols, filter = filter_query)$standardize()
    }
  )
)
