#' AWWID table class
#'
#' @return a `AwwidTable` R6 class
#' @keywords internal
awwid_tbl = R6::R6Class(
  classname = "awwid_tbl",
  ...
  public = list(
    ...
    #' @description
    #' Print method for the 'awwidTbl' class
    print = function() {
    ...
    #' @description
    #' Standardize a awwidTbl object by converting all measurement columns
    #' from imperial units to metric equivalents, and renaming some column
    #' names to avoid name collisions when joining tables
    standardize = function() {
      df = private$standardize_default(self$data)
      switch(
        self$name,
        analysisitems = private$standardize_analysisitems(df),
        boreholes = private$standardize_boreholes(df),
        wellcasinglogs = private$standardize_wellcasinglogs(df),
        chemicalanalysis = private$standardize_chemicalanalysis(df),
        drillingcompanies = private$standardize_drillingcompanies(df),
        elements = private$standardize_elements(df),
        lithologies = private$standardize_lithologies(df),
        otherseals = private$standardize_otherseals(df),
        pumptests = private$standardize_pumptests(df),
        pumptestitems = private$standardize_pumptestitems(df),
        screens = private$standardize_screens(df),
        wells = private$standardize_wells(df),
        wellmaterialslogs = private$standardize_wellmaterialslogs(df),
        welldecommissioningdetails = private$standardize_welldecommissioningdetails(df),
        wellreports = private$standardize_wellreports(df),
        perforations = private$standardize_perforations(df)
      )
    }
  ),
  ...
  private = list(
    ...
    standardize_default = function(x) {
    ...
    standardize_wellcasinglogs = function(x, ...) {
      # feet to metres columns
    ...
    standardize_chemicalanalysis = function(x) {
    ...
    standardize_drillingcompanies = function(x) {
      cols = grep("lastwellidused", names(x))
      for (j in cols) data.table::set(x, j = j, value = as.integer(x[[j]]))
      return(x)
    },

    standardize_elements = function(x) {
      cols = grep("decimalplaces", names(x))
      for (j in cols) data.table::set(x, j = j, value = as.integer(x[[j]]))
      return(x)
    },

    standardize_lithologies = function(x) {
      # some checks
    ...
    standardize_otherseals = function(x) {
    ...
    standardize_pumptests = function(x) {
    ...
    standardize_pumptestitems = function(x) {
    ...
    standardize_screens = function(x) {
    ...
    standardize_wellmaterialslogs = function(x) {
      cols = grep("fromdepth|todepth", names(x))
      for (j in cols) data.table::set(x, j = j, value = private$ft_to_m(x[[j]]))
      return(x)
    },

    standardize_welldecommissioningdetails = function(x) {
    ...
    standardize_wellreports = function(x) {
      # numeric
      cols = c(
        "plugmaterialamount",
        "annularsealamount",
    ...
    standardize_perforations = function(x) {
      # feet to metres
      cols = grep("from|to", names(x))
      for (j in cols) data.table::set(x, j = j, value = private$ft_to_m(x[[j]]))
